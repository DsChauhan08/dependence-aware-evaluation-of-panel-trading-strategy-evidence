...
\midrule
French momentum deciles, rank-threshold panel & 0.447 & 7.54e-05 & N/A & 0.447 & Permutation uninformative; remaining applicable checks pass \\
...
\par\footnotesize\emph{Note:} p+ denotes a one-sided positive-Sharpe p-value. Row-naive p+ treats selected asset-date rows as independent. Date-IID, Date-HAC, and Moving-block p+ operate on the date-level portfolio return series. Sharpe UVIF is the diagnostic variance ratio between date-level long-run Sharpe uncertainty and row-pooled IID Sharpe uncertainty.
\end{table}
