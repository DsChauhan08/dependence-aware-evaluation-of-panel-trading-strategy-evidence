...
\title{Dependence-Aware Evaluation of Panel Trading Strategy Evidence}
\author{Dhananjay S. Chauhan\\\texttt{dschauhan08.me@gmail.com}}
...
\begin{abstract}
Trading-strategy evidence is often produced from symbol-date panels, but
the tradable object is a date-level portfolio return.  Treating selected
rows as independent can therefore overstate evidence when same-date rows
share market, factor, liquidity, or volatility shocks and when the
resulting date returns are serially dependent.  This paper proposes a
double-robust inference framework for panel trading evidence: date
aggregation, HAC-delta inference, and dependent bootstrap resampling
jointly address cross-sectional and serial dependence in the
date-return series.  The main theoretical object is the Sharpe Unified
Variance Inflation Factor (Sharpe UVIF), defined as the ratio of the
long-run delta-method variance of the date-level Sharpe estimator to the
row-pooled IID delta-method variance.  It is the Sharpe-ratio analogue of
a design effect.  A companion Effective Information Rate (EIR) reports
the fraction of nominal row-level information that remains nonredundant
under an equicorrelation diagnostic.  The empirical analysis is a
reproducible evaluation of public factor-zoo candidates using Kenneth
French and AQR data.  The results show that benchmark momentum survives
the dependence-aware boundary, while weaker panel claims can lose most
of their nominal row-level information once the audit is applied.
\end{abstract}

\noindent\textbf{Keywords:} panel data, Sharpe ratio, bootstrap,
data snooping, multiple testing, factor alpha, trading strategy evaluation

\noindent\textbf{JEL Classification:} C12, G11, G17
...
\subsection{Solution: Double-Robust Inference}
...
into a pre-specified double-robust decision record: date aggregation is
robust to cross-sectional clustering, and HAC-delta plus moving-block or
stationary bootstrap inference is robust to serial dependence in the
...
The empirical evidence is framed as an evaluation of common public
factor-zoo candidates, not as a toy positive-control example.  It asks
...
The protocol is a double-robust inference framework rather than a single
test.  Its first robustness channel is cross-sectional: all evidence is
...
row multiplication.  Its second robustness channel is serial: all
...
defined by \(\hat\mu-\widehat{TO}c=0\).  For true traded security panels,
this screen should be supplemented with a nonlinear market-impact model,
such as a square-root impact stress in the spirit of
\citet{almgren2001optimal} and the broader market-impact literature
\citep{bouchaud2008markets}.  The public sorted-portfolio examples do not
contain average daily volume or order-size information, so they cannot
support a capacity claim.

\begin{theorem}[Asymptotic capacity frontier under square-root impact]
\label{thm:capacity-frontier}
...
\section{Public Factor-Zoo Evaluation Results}
\label{sec:empirical}

The public experiment is a pre-specified candidate campaign rather than
...
factor-zoo benchmarks, but they are pre-aggregated series rather than
...
Failed public data sources are reported instead of silently removed.
...
Table \ref{tab:phantom-audit} gives the artifact-backed phantom-alpha
...
number came from a stale rank-threshold stress candidate, not from the
...
The phantom-alpha summary is intentionally artifact-backed rather than
...
\section{Monte Carlo Design and Simulation Results}
...
release scripts, and SHA256 provenance.  The release target is a
GitHub/Zenodo archive with code under Apache-2.0 and manuscript/public
aggregate artifacts under CC-BY 4.0 unless a target venue requires
different terms.  The arXiv source package is generated from the same
rendered manuscript artifacts.  Proprietary predictions, raw trades,
private tickers, production feature definitions, and unrelated workspace
