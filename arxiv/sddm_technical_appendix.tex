...
\title{Technical Appendix: Dependence-Aware Evaluation of Panel Trading Strategy Evidence}
...
\section{Capacity Frontier Proof}
...
\[
  ...
  - z_{\alpha}\mathcal M\sigma_{\mathrm{row}}/\sqrt{T}.
\]
Rearranging yields the capacity frontier reported in the main text.  If
...
\section{Higher-Moment UVIF and EIR Details}
...
\]
Under the same equicorrelated Gaussian approximation used for the
mean-return design effect,
\[
  \mathrm{EIR}_t(\rho)\approx \frac{1}{1+(n_t-1)\rho}.
\]
Thus an EIR of \(0.111\) means that roughly \(1-0.111=0.889\), or 89
percent, of nominal row-level information is redundant under the
