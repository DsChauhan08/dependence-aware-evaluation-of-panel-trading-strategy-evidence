...
tables, and reproducibility artifacts omitted from the main manuscript
for brevity.  It supports the manuscript's proof details, calibration,
robustness, and reproducibility record; it is not an independent
empirical claim.
...
\[
  ...
  d_R=\nabla f(\mu_R,m_{2,R}).
\]
The annualization factor \(A_f\) is source-specific: the public registry
...
strategy; it verifies that the registry-defined reporting rule can
...
turnover-stress convention as the public artifacts.  The replication
...
\[
  ...
  \qquad s_{t,i}=z_{t,i}.
\]
The audit then applies the same signed-row construction, date
aggregation, HAC-delta inference, Romano-Wolf menu adjustment,
same-date permutation, and 5 bps cost stress used in the public
artifacts.
...
These tables are audit logs rather than primary evidence; the main text
...
Rendered artifacts in the submitted package use the following SHA256
identifiers:
\begin{quote}\scriptsize
\noindent registry \texttt{code/campaign\_registry.json}:\\
