...
Please consider my manuscript, "A Practical Sharpe-Ratio Diagnostic for Portfolio Backtests," for publication in the Journal of Asset Management.
...
The contribution is deliberately applied. The article does not propose a new trading rule or asset-pricing anomaly. Instead, it gives a backtest-review checklist: aggregate selected rows to date-level portfolio returns, conduct Sharpe inference on the date series using robust procedures, compare row-pooled and date-level uncertainty, apply fixed-menu and placebo checks where applicable, and report turnover-cost sensitivity.
